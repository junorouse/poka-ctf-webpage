...
// todo get json and render it.


function showChall(event) {
    closeLogin();
    $('#flag')[0].setAttribute('class', "input");
    $('#flag')[0].value = '';
    ...
    $.ajax({
        type: 'get',
        url: '/api/get_detail/'+x.getAttribute('class').split("square")[1],
        success: function(data) {
            data = JSON.parse(data);
            $('#real_title').html(data.chall_title + "(" + data.chall_score + ")");
            $('#real_detail').html(data.chall_detail);
            $('#chall_id').val(data.chall_id);
        }
    });

    // document.getElementById("wtf").style.opacity = 0.5;
    document.getElementById("challenge_view").style.display = "block";
}

function closeChall() {
    // document.getElementById("wtf").style.opacity = 1;
    document.getElementById("challenge_view").style.display = "none";
}
...
function doAuth(idx, flag) {
    $.ajax({
        type: 'post',
        url: '/api/auth',
        data: 'chall_id='+idx+"&flag="+encodeURIComponent(flag),
        success: function(data) {
            data = JSON.parse(data);
            k = $('#flag');
            if (data.status) {
                $('#flag')[0].setAttribute('class', "input is-success");
                $('#flag')[0].value = 'Success !';
                location.reload();
            } else {
                $('#flag')[0].setAttribute('class', "input is-danger");
                $('#flag')[0].value = data.msg;
            }
        }
    })
}
...
$('#flag').keyup(function(e) {
     if (e.keyCode == 13) {
        doAuth($('#chall_id').val(), $('#flag').val());
    }
});


function refreshChalls() {
    $.ajax({
        type: 'get',
        url: '/api/get_challs',
        success: function(data) {
...
var endDate = "september 22, 2017 21:00:00";
